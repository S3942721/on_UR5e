...
set(_exported_dependencies "geometry_msgs;shape_msgs;rclcpp;sensor_msgs;std_msgs;trajectory_msgs;visualization_msgs;eigen3_cmake_module;Eigen3;eigen_stl_containers;interactive_markers;rviz_common;rviz_rendering;rviz_default_plugins;rviz_ogre_vendor")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to rviz_visual_tools_DEFINITIONS, rviz_visual_tools_INCLUDE_DIRS,
# rviz_visual_tools_LIBRARIES, and rviz_visual_tools_LINK_FLAGS.
# Additionally collect the direct dependency names in
# rviz_visual_tools_DEPENDENCIES as well as the recursive dependency names
# in rviz_visual_tools_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(rviz_visual_tools_DEPENDENCIES ${_exported_dependencies})
  set(rviz_visual_tools_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(rviz_visual_tools_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(rviz_visual_tools_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(rviz_visual_tools_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(rviz_visual_tools_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(rviz_visual_tools_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND rviz_visual_tools_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
